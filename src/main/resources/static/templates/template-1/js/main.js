    ...
    if (rsvpForm) {
        rsvpForm.addEventListener('submit', function (e) {
            ...
            if (nameInput && btn) {
                ...
                // Submit to ToiLink API
                const attendValue = document.getElementById('rsvpAttend')?.value || '';
                const guestsValue = parseInt(document.getElementById('rsvpGuests')?.value || '1', 10);
                const wishValue   = document.getElementById('rsvpWish')?.value || '';
                const rsvpStatus  = attendValue === 'yes' ? 'ATTENDING' : 'DECLINED';
                const rsvpSlug    = C.rsvp && C.rsvp._slug;
                const rsvpToken   = C.rsvp && C.rsvp._guestToken;
                ...
                if (rsvpSlug) {
                    fetch('/api/public/events/' + rsvpSlug + '/rsvp', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            guestToken: rsvpToken || null,
                            name:       name,
                            status:     rsvpStatus,
                            groupSize:  guestsValue || 1,
                            comment:    wishValue || null,
                        }),
                    })
                    .then(r => {
                    ...
                    .catch(() => {
                        btn.textContent = (C.rsvp && C.rsvp.form && C.rsvp.form.btnText) || 'Отправить';
                        btn.disabled = false;
                    });
                } else {
                    // Fallback: show success immediately (dev mode without slug)
