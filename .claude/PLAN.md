...
### ✅ Шаг 3 — DTO + Exception handling (DONE)
- Request records: CreateEventRequest, UpdateEventRequest, CreateGuestRequest, RsvpRequest (Bean Validation)
- Response records со static from(): TemplateResponse, EventResponse, EventPublicResponse, GuestResponse
- NotFoundException (со static factories), BadRequestException
- GlobalExceptionHandler: NOT_FOUND/BAD_REQUEST/VALIDATION_ERROR/500
- Jackson: write-dates-as-timestamps=false, fail-on-unknown-properties=false

### ✅ Шаг 4 — SlugService (DONE)
- Миграция 007: добавлены person1, person2 в events
- SlugGenerator: транслитерация кириллицы (ru + ky) → латиница + random fallback
- SlugService: генерирует уникальный slug из person1/person2, обрабатывает коллизии (base-2, base-3...)
- Event entity и CreateEventRequest обновлены

### ✅ Шаг 5 — Template API (DONE)
- `TemplateService` + `TemplateController`
- `GET /api/organizer/templates` — список активных шаблонов
- Данные уже есть из seed миграции

### ✅ Шаг 6 — Event CRUD (DONE)
...
### ✅ Шаг 9 — OG endpoint (Thymeleaf) (DONE)
