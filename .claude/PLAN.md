...
### Шаг 1 — Liquibase + DB
- Добавить Liquibase в pom.xml
- `application.yaml`: datasource, liquibase config
- Создать `db/changelog/db.changelog-master.yaml`
- Миграции: users, templates, events, guests, rsvp_responses
- Seed: 1-2 шаблона с `blocks_schema`

### Шаг 2 — Entity + Repository
- JPA entities для всех таблиц (Lombok: @Data / @Builder / @NoArgsConstructor)
- Spring Data repositories (только нужные методы: findBySlug, findByEventId и т.д.)
